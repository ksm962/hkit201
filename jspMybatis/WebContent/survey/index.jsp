...
function suntaek_sakje(value1){
	...
	$.ajax({
		type:"post",
		data:param,
		url:"${path}/survey_servlet/sakje.do",
...
function GosakjeProc(value1){
	...
	var param ={
		...
		$.ajax({
			type:"post",
			data: param,
			url:"${path}/survey_servlet/sakjeProc.do",
			success:function(){ //콜백함수(서버에서 처리가 완료된 후 실행되는 코드)
				suntaek_page('1');	
			}
		});
...
function Gomodfiy(){	
	...
	$.ajax({
		type:"post",
		data:param,
		url:"${path}/survey_servlet/modfiy.do",
		success:function(data){
			$("#result").html(data);	
		}
	})
...
function GomodfiyProc(){
	...
	if(confirm('수정하시겠습니까?')){
		$.ajax({
			type:"post",
			data:param,
			url:"${path}/survey_servlet/modfiyProc.do",
			success:function(){ //콜백함수(서버에서 처리가 완료된 후 실행되는 코드)
				suntaek_page('1');	
			}
		});
